...
describe("DistributedLockService", () => {
  ...
  test("Given Valid lock key, TTL, and cache instance When Instantiating a new DistributedLockService Then The lock service should be created with the correct properties", () => {lockService = new DistributedLockService("test-lock", 1000, mockCache);
  ...
  test("Given Valid lock key, undefined TTL, and cache instance When Instantiating a new DistributedLockService Then The lock service should use the default TTL value", () => {lockService = new DistributedLockService("test-lock", undefined, mockCache);

      expect(lockService.ttl).toBe(60000);
    });

  test("Given Valid lock key, TTL, and null cache instance When Instantiating a new DistributedLockService Then The lock service should allow null cache", () => {lockService = new DistributedLockService("test-lock", 1000, null);

      expect(lockService.cacheLayer).toBeNull();
    });

  test("Given Empty lock key When Instantiating a new DistributedLockService Then An error should be thrown", () => {expect(() => new DistributedLockService("", 1000, mockCache)).toThrow("Lock key must be a non-empty string");
    });

  test("Given Non-string lock key (number) When Instantiating a new DistributedLockService Then An error should be thrown", () => {expect(() => new DistributedLockService(123, 1000, mockCache)).toThrow("Lock key must be a non-empty string");
    });

  test("Given Invalid TTL values (negative or non-numeric) When Instantiating a new DistributedLockService Then An error should be thrown", () => {expect(() => new DistributedLockService("test-lock", -1, mockCache)).toThrow("TTL must be a positive number");
  ...
  test("Given cache without disconnect method When disconnect called Then should handle gracefully", async () => {
    ...
    describe("getStatus()", () => {
  test("Given active lock When getStatus called Then should return comprehensive status", () => {lockService.isLocked = true;
    ...
    describe("getLockKey()", () => {
  test("Given Test setup for Given lock service When getLockKey called Then should return lock key When Action being tested Then Expected outcome", () => {expect(lockService.getLockKey()).toBe("test-lock");
      });
    });

    describe("getLockTtl()", () => {
  test("Given Test setup for Given lock service When getLockTtl called Then should return lock TTL When Action being tested Then Expected outcome", () => {expect(lockService.getLockTtl()).toBe(1000);
      });
    });
  });
