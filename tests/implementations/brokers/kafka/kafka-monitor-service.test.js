...
describe("KafkaMonitorService", () => {
  ...
  test("Given Test setup for should initialize with configuration When Action being tested Then Expected outcome", () => {expect(monitorService._groupId).toBe("test-group");
  ...
  test("Given Test setup for should return 'kafka' When Action being tested Then Expected outcome", () => {expect(monitorService.getBrokerType()).toBe("kafka");
    });
  });
