...
logger.logInfo(name); // ['Ryan', 'McDermott'];
```
...
logger.logInfo(name); // 'Ryan McDermott';
logger.logInfo(newName); // ['Ryan', 'McDermott'];
```
...
logger.logInfo(`Employee name: ${employee.getName()}`); // Employee name: John Doe
delete employee.name;
logger.logInfo(`Employee name: ${employee.getName()}`); // Employee name: undefined
```
...
class Car {
  ...
  save() {
    logger.logInfo(this.make, this.model, this.color);
  }
}
...
or react to said error. Logging the error to the console (`logger.logInfo`)
...
try {
  ...
  // One option (more noisy than logger.logInfo):
  console.error(error);
  // Another option:
